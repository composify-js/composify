...
This guide shows how to integrate Composify into a Next.js project. We'll assume you already have a Next.js project up and running. If not, start with the [Next.js getting started guide](https://nextjs.org/docs/app/getting-started/) first.

Prerequisite: Ensure your mock API is running on port 9000. (See [Prerequisites](/docs/tutorial/prerequisites))
...
Let's assume you have a design system in your project. These are standard React components. You do not need to change them.

<Tabs items={['Heading', 'Body', 'VStack', 'HStack']}>
  <Tabs.Tab>
    ```tsx
...
Composify needs to know which components are editable. We create a Catalog file to map your components to the visual editor. This keeps your metadata separate from your logic.

<Tabs items={['HeadingCatalog', 'BodyCatalog', 'VStackCatalog', 'HStackCatalog']}>
  <Tabs.Tab>
    ```tsx
