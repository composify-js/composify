...
A radio prop lets users pick one option from a set, shown as a group of radio buttons.
...
Defines the list of choices for the radio group. Each option needs a label (what users see) and a value (what gets passed to the component).
...
You can override how the radio group is displayed by providing a custom render function.
...
Note that when you use a custom render, `options` should only include the values. Don't include labels — the render function is in charge of how each option looks.
