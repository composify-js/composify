...
Defines the list of choices for the radio group.
Each option needs a label (what users see) and a value (what gets passed to the component).
...
Note that when you use a custom render, `options` should only include the values.
Don't include labels — the render function is in charge of how each option looks.
