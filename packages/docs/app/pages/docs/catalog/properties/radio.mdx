...
- `label`: The text displayed next to the button
- `value`: The value passed to your component when selected
...
:::warning
When using a custom `render` function, the `options` array should only contain raw values (e.g., `['left', 'center', 'right']`). Your `render` function handles the labels.
:::
